...
**版本：** v1.8（2026-09-14 晚；0914 派工裁決：主畫面接入定案＋M2 必要項擴大為 B1＋B2＋B4）
**建立日期：** 2026-08-22
**規劃：** aaaaa
**守門與合入：** dernoson（主編）
**上游來源：** 主編提出並確認的 ROADMAP v0.2（決策層原始文件，未公開；其內容已完整拆進本檔與 `detail/`）
**狀態總覽：** **M1 成立。** L1 解鎖維持（PR #40）。**B1 9/6 切片維持**（PR #43）。**W0907：** toby **GridCanvas 只讀已合入**（PR #46）；shirone MachineCard **已合入**（PR #41）；aaaaa **layoutStore 契約已交、PR #45 仍開**。harry viewport PR #47 待合。**W0914（本週）：** 主畫面接入定案（佈局視角改掛 `GridCanvas`，舊 `FactoryCanvas` 保留但非入口）；**擺放／選取仍鎖至 9/21**；**M2 必要項經主編裁定擴大為 B1＋B2＋B4**。詳見 §1.5／§8／§9／§12 與 [WEEK_20260914](../work_dispatch/WEEK_20260914.md)。
...
| **佈局渲染層** | **確認提前開發**佈局視角 **SVG 自建**（拔 Vue Flow；流程視角續用 Vue Flow）。**L1 打底已合入並宣告解鎖**（PR #40）；**只讀殼已合入**（PR #46）。**主畫面接入（2026-09-14 定案）：** 佈局視角改掛 `GridCanvas`，**舊 `FactoryCanvas` 保留不刪、不再是入口**，容器留切換開關當退路。擺放／選取解鎖排 9/21。見 **§1.6**、[EARLY_START](../aaaaa/LAYOUT_REWRITE_EARLY_START_0831.md) |
| **佈局 L1 主責** | **aaaaa**；時數與 B1 衝突時 **L1＞B1**。dernoson 只裁示／合入／守閘，不兼功能（規則 17） |
| 關鍵路徑限制 | 月底門檻必要條件只派 `risk ≤ 中`。**9 月起程式必要實質只剩 aaaaa**（shirone 已轉 L3）。原「渲染層落地前不把必要項押在 L2」一條**經主編 2026-09-14 裁定放寬**：M2 必要項擴大為 B1＋B2＋B4，B2 的容器接入由 toby 承擔；harry／shirone 的塊仍不列必要（§11、[WEEK_20260914](../work_dispatch/WEEK_20260914.md) §2.1） |
...
R-B 擺放（9/6→9/27）──── 使用者可見：真機器選單、擺放、選取、刪／轉
      ...
      │                      ▼
      │                 09/14：殼接主畫面（只讀）＋viewport；store 合入
      │                 09/21：擺放／選取解鎖（B2／B4 接線）
      ▼
...
| M2 擺放 | 2026-09-27 | 拉多種真機器；點選看資訊；單刪；可轉 90° | B1、B2、B4 | **2026-09-14 主編裁定：三項皆為必要**（覆寫 v1.6「只硬綁 B1」）。承擔者＝aaaaa（store）＋toby（容器接入）；harry／shirone 的支撐塊不列必要。9/20 檢查點未過才依 §11 降級（D0 書面確認）。見 [WEEK_20260914](../work_dispatch/WEEK_20260914.md) §2.1 |
...
| **優先序** | **① store 合入（最優）→ ② 主畫面接入 → ③ L3 收尾／加分**（2026-09-14）。擺放／選取**禁止**搶跑至 9/21 |
| **卡住誰？** | **等 9/21 解鎖：** B2 落子、B4 選取接線、B3／B5、C\* 畫布。**本週進行中：** store 合入（#45）、主畫面接入（T1）、viewport（#47）。**不卡：** B1（9/6 切片已合入）、D1–D3、L3 卡片／面板元件／paper |
| **關鍵路徑** | 寬度＝1 仍在 aaaaa（#45 合入），其後轉到 toby（容器接入）。只讀殼已進 master，但**接上主畫面前不能假裝 B2 已解** |
| **誰做 L1？** | **aaaaa**（[W0831-A0](../work_dispatch/aaaaa/0831/W0831-A0_layout_l1_foundation.md)）**已交**；store＝[W0907-A0](../work_dispatch/aaaaa/0907/W0907-A0_layout_store_model.md) 已交、[W0914-A0](../work_dispatch/aaaaa/0914/W0914-A0_layout_store_land.md) **本週收尾合入** |
| **L2 解鎖** | L1 解鎖維持（PR #40）。**只讀殼已合入**（toby #46）。**9/14 新增放行：** L2 可將只讀殼與 viewport **接進主畫面**（[T1](../work_dispatch/toby/0914/W0914-T1_main_view_integration.md)／[H1](../work_dispatch/harry/0914/W0914-H1_viewport_into_canvas.md)），但**互動仍限只讀**；擺放／選取排 9/21，超出即退回 |
| **L2 檔案切分** | 同週一檔一人：toby＝`LayoutView.vue`／`GridCanvas.vue`／`MainLayout.vue`；harry＝`useGridViewport.ts`／dev 頁／`router`。**import 對方的檔可以，改對方的檔退回** |
| **誰不做？** | dernoson 不兼功能；L3 不碰 layout 演算法；本週無人動 `src/editor/inspector/*`（B4 接線排 9/21） |
...
- [!] **R-B2** 擺放鏈 L2 串接：工具列意圖 → drop／click → **只呼叫** `placeDevice`；預覽佔格讀真實 machine size
  - 細項：[detail/B2_placement_chain.md](./detail/B2_placement_chain.md)（**只讀殼已合入** #46；layoutStore **#45 待合**。**W0914：主畫面接入中**（[T1](../work_dispatch/toby/0914/W0914-T1_main_view_integration.md)），落子鏈本身**排 9/21**。**9/14 起本項為 M2 必要項**，承擔者＝aaaaa（store）＋toby（容器））
...
- [~] **R-B4** 選取與設備資訊面板：選取 → L2 攤成 plain props → L3 顯示
  - 細項：[detail/B4_selection_inspector.md](./detail/B4_selection_inspector.md)（PR #33 名稱／佔格／耗電；MachineCard #41 已合、仍非選取鏈。**W0914：** 標的確認為本項；shirone 交 **L3 呈現元件**（[S1](../work_dispatch/shirone/0914/W0914-S1_device_info_panel.md)，吃 §4.1 攤平 props）；**L2 攤平層與 Inspector 接線排 9/21**，MBD 9/21 起接空狀態文案。**9/14 起本項為 M2 必要項**）
...
- [ ] **R-E3** 備援與人力調度：可投入時間不穩定者不承擔門檻必要條件、每週配對名額 ≤ 2、已知空窗不派工
  ...
  - **2026-09-14 晚 決策層備註（0914 派工裁決摘要）：** ①主畫面接入＝改掛 GridCanvas、舊殼保留非入口；②本週互動仍只讀，擺放／選取排 9/21；③L2 owner：toby＝容器、harry＝viewport，同檔退回後到者；④右側面板標的＝R-B4，shirone 交 L3 呈現、MBD 9/21 接空狀態；⑤MBD 維持暫停至 9/20；⑥`ToolbarPanel.vue` 本週對 goodmorning 解鎖（限視覺）；⑦L2 不再以 Storybook 驗收、L3 保留；⑧**M2 必要項擴大為 B1＋B2＋B4**。
...
| **09/20** | **主畫面接入（只讀）＋store 合入** | **本週派工：** `pnpm dev` 首頁佈局視角畫的是 `GridCanvas`（可切回舊殼）＋#45 進 master＋#47／#48 合入；L3 交設備資訊面板元件。**檢查點：** 兩項未達成 → B2／B4 依 §11 降級（D0 書面）。見 [WEEK_20260914](../work_dispatch/WEEK_20260914.md) |
| 09/27 | **M2 門檻** | **必要＝B1＋B2＋B4**（9/14 裁定）。B2 落子鏈與 B4 選取接線在 9/21 週解鎖並收；B3／B5 未完本日補或改期 |
...
|----|----------|----------|----------|------|
| R-B2 | L1 已宣告；只讀殼 **#46 已合入**。本項（`placeDevice` 落子鏈）仍等 **store 合入（#45）＋新殼在主畫面上站穩** | **aaaaa（#45）**；**toby（主畫面接入 W0914-T1）** | store 進 master ＋ 佈局視角畫的是新殼 → **9/21 開落子** | `[!]` **本週接入中**（只讀）。**已列 M2 必要項**（9/14 裁定）；9/20 為降級檢查點 |
...
- ~~**渲染層落地前，不把任何月底門檻的必要項押在 L2**（2026-08-25）~~ → **2026-09-14 放寬：** M2 必要項含 B2／B4，其中 **B2 的容器接入押在 toby（risk 中、L2 小塊 ALLOW）**；`risk 中且該域不穩` 者（harry、shirone）的塊仍**不列必要**，且各自寫明頂替。放寬僅限 M2，M3 以後回到本條原則另議
- **舊 Vue Flow 佈局畫布：只維護、不加深；且在新殼站穩前不刪檔**——換入口即可（2026-09-14），刪除須另開廢除工項
- **檔案鎖以當週 `WEEK_*.md` §3 為準**；鎖可按週轉移（例：`ToolbarPanel.vue` 0907 對全員硬鎖 → 0914 開放給 goodmorning 限視覺）。改到不屬於自己的檔即退回，不論內容好壞
...
| 版 | 日期 | 說明 |
|----|------|------|
| **v1.8** | **2026-09-14 晚** | **0914 派工裁決：** 主畫面接入定案（換入口、舊殼保留非入口、留切換）；**M2 必要項擴大為 B1＋B2＋B4** 並在 §1.5／§11 寫明承擔與放寬範圍；L2 檔案切分（toby 容器／harry viewport）；`ToolbarPanel.vue` 鎖轉給 goodmorning（限視覺）；B4 標的與分工（shirone L3／MBD 9/21 空狀態）；§8 09/20 改為接入檢查點 |
